...
class FrankxPostProcessor {
  ...
  internal FrankxPostProcessor(SystemFranka system, Program program) {
    ...
    if (program.MultiFileIndices.Count > 1)
      program.Warnings.Add(
          "Multi-file input not supported on Franka Emika robots");
  }

  List<string> Program() {
    var code = new List<string> { $@"from argparse import ArgumentParser
...
" };
    ...
    foreach (var systemTarget in _program.Targets) {
      ...
      if (target is JointTarget joint) {
        ...
        code.Add(
            $"  motion = JointMotion([{j[0]:0.#####}, {j[1]:0.#####}, {j[2]:0.#####}, {j[3]:0.#####}, {j[4]:0.#####}, {j[5]:0.#####}, {j[6]:0.#####}])");
        ...
        // var transform = _system.BasePlane.ToInverseTransform() *
        // Transform.PlaneToPlane(tcp, plane); plane = transform.ToPlane();

        var elbow =
            target.External.Length > 0 ? $", {target.External[0]:0.#####}" : "";

        var zone = target.Zone.Distance.ToMeters();
        code.Add(
            $"    Waypoint({Affine(plane)}, {speed:0.#####}, {target.Zone.Name}{elbow}),");
      }
