...
class KRLPostProcessor {
  ...
  internal KRLPostProcessor(SystemKuka system, Program program) {
    ...
    for (int i = 0; i < _system.MechanicalGroups.Count; i++) {
      var groupCode = new List<List<string>> { MainFile(i), DatFile(i) };
  ...
  List<string> DatFile(int group) {
    ...
    var code = new List<string> { $@"&ACCESS RVP
&REL 1
DEFDAT {_program.Name}_{groupName} PUBLIC" };
    ...
    foreach (var frame in attributes.OfType<Frame>().Where(
                 t => !t.UseController))
      code.Add(Frame(frame));

    foreach (var speed in attributes.OfType<Speed>())
      code.Add(
          $"DECL GLOBAL REAL {speed.Name} = {speed.TranslationSpeed.ToMeters():0.#####}");
...
$APO.CPTP = 100" };
  ...
  List<string> SrcFile(int file, int group) {
    ...
    int end = (file == _program.MultiFileIndices.Count - 1)
                  ? _program.Targets.Count
                  : _program.MultiFileIndices[file + 1];
...
DEF {_program.Name}_{groupName}_{file:000}()" };
    ...
    for (int j = start; j < end; j++) {
      ...
      if (target.Zone.IsFlyBy &&
          (currentZone is null || target.Zone != currentZone)) {
          ...
          if (programTarget.IsJointMotion) {
            ...
            if (Abs(currentPercentSpeed - percentSpeed) > UnitTol) {
              ...
              if (systemTarget.DeltaTime > UnitTol)
                code.Add(
                    $"$VEL_AXIS[{programTarget.LeadingJoint + 1}] = {percentSpeed * 100:0.###}");
      ...
      if (programTarget.IsJointTarget) {
        ...
        double[] jointDegrees = jointTarget.Joints.Map(
            (x, i) =>
                _system.MechanicalGroups[group].Robot.RadianToDegree(x, i));

        var pos =
            $"A1 {jointDegrees[0]:0.####},A2 {jointDegrees[1]:0.####},A3 {jointDegrees[2]:0.####},A4 {jointDegrees[3]:0.####},A5 {jointDegrees[4]:0.####},A6 {jointDegrees[5]:0.####}";
        ...
        case Motions.Joint: {
          ...
          {
            double[] jointDegrees = programTarget.Kinematics.Joints.Map(
                (x, i) =>
                    _system.MechanicalGroups[group].Robot.RadianToDegree(x, i));
            int turnNum = 0;
            for (int i = 0; i < 6; i++)
        ...
        default:
          throw new ArgumentException(
              $" Motion '{cartesian.Motion}' not supported.");
        }
      }
      ...
      string Motion(string motion, string name, string pos,
                    string? bits = null) {
        var externalCustom = target.ExternalCustom;

        if (externalCustom is null) {
          double[] values =
              _system.MechanicalGroups[group].RadiansToDegreesExternal(target);
          string external = "";
  ...
  string ExternalSpeed(ProgramTarget target) {
    ...
    var externalSpeedCode =
        $"$VEL_EXTAX[{target.LeadingJoint + 1 - 6}] = {percentSpeed * 100:0.###}";
  ...
  string Tool(Tool tool) {
    ...
    var loadTxt =
        $"DECL GLOBAL LOAD {tool.Name}_L = {{M {tool.Weight:0.####},CM {{{GetXyzAbc(centroid.X, centroid.Y, centroid.Z, 0, 0, 0)}}},J {{X 0,Y 0,Z 0}}}}";
    return $"{toolTxt}\r\n{loadTxt}";
  }
