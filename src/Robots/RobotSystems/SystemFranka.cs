...
public class SystemFranka : CobotSystem {
  internal SystemFranka(string name, RobotFranka robot, IO io, Plane basePlane,
                        Mesh? environment)
      : base(name, Manufacturers.FrankaEmika, robot, io, basePlane,
             environment) {
    Remote = new RemoteFranka();
    RobotJointCount = 7;
  }

  static Plane QuaternionToPlane(double x, double y, double z, double q1,
  ...
  public override Plane NumbersToPlane(double[] numbers) {
    ...
    return QuaternionToPlane(n0, n1, n2, numbers[3], numbers[4], numbers[5],
                             numbers[6]);
  }

  internal override List<List<List<string>>>
  Code(Program program) => new FrankxPostProcessor(this, program).Code;
