...
public class SystemUR : CobotSystem {
  internal SystemUR(string name, RobotUR robot, IO io, Plane basePlane,
                    Mesh? environment)
  ...
  static double[] PlaneToAxisAngle(ref Plane plane) {
    ...
    if ((Abs(t.M01 - t.M10) < epsilon) && (Abs(t.M02 - t.M20) < epsilon) &&
        (Abs(t.M12 - t.M21) < epsilon)) {
      // singularity found
      // first check for identity matrix which must have +1 for all terms
      //  in leading diagonal and zero in other terms
      if ((Abs(t.M01 + t.M10) < epsilon2) && (Abs(t.M02 + t.M20) < epsilon2) &&
          (Abs(t.M12 + t.M21) < epsilon2) &&
          (Abs(t.M00 + t.M11 + t.M22 - 3) < epsilon2)) {
        // this singularity is identity matrix so angle = 0
        return new double[] {
          plane.OriginX, plane.OriginY, plane.OriginZ, 0, 0, 0
      ...
      if ((xx > yy) && (xx > zz)) { // m.M00 is the largest diagonal term
        if (xx < epsilon) {
          x = 0;
          y = 0.7071;
          z = 0.7071;
      ...
      } else if (yy > zz) { // m.M11 is the largest diagonal term
        if (yy < epsilon) {
          x = 0.7071;
          y = 0;
          z = 0.7071;
      ...
      } else { // m.M22 is the largest diagonal term so base result on this
        if (zz < epsilon) {
          x = 0.7071;
          y = 0.7071;
          z = 0;
    ...
    vector.Unitize();
    vector *= angle;
    return new double[] { plane.OriginX, plane.OriginY, plane.OriginZ, vector.X,
                          vector.Y,      vector.Z }; // return 180 deg rotation
  }

  static Plane AxisAngleToPlane(double x, double y, double z, double vx,
                                double vy, double vz) {
  ...
  internal override List<List<List<string>>>
  Code(Program program) => new URScriptPostProcessor(this, program).Code;
  ...
  internal static string CreateUrp(IProgram program) {
    ...
    var isESeries =
        ur.Robot.Model.EndsWith("e", StringComparison.OrdinalIgnoreCase);
